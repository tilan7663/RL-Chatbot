...
def make_batch_X_elmo(batch_X, n_encode_lstm_step, noise=False):
    # todo: adding noise to data generation
    tokens_input = []
    tokens_length = []
    for i in range(len(batch_X)):
        tokens = batch_X[i]
    ...
    return tokens_input, tokens_length
...
def train():
    ...
    with g1.as_default():
        ...
        tf_tokens_input, tf_tokens_length, tf_actions, tf_feats = model.build_generator()
    ...
    with g2.as_default():
        reversed_model = Seq2Seq_chatbot(
            ...
            lr=learning_rate)
        _, _, tf_reversed_tokens_input, tf_reversed_tokens_length, caption, caption_mask, reverse_inter = reversed_model.build_model()
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch in range(sb, n_batch):
            ...
            tokens_input, tokens_length = make_batch_X_elmo(
                                          batch_X=copy.deepcopy(batch_X), 
                                          n_encode_lstm_step=n_encode_lstm_step)
            ...
            if training_type == 'pg':
                ...
                # generate actions by running the policy
                ...
                actions = []
                actions_list = []
                # Map the actions to sentences, size should match the batch
                for i in range(len(action_word_indexs)):
                    ...
                    # action_list contains the tokenized sentence
                    actions_list.append(action.split())

                # vectorize actions sentences and convert it to matrix
                # action_feats = make_batch_X(
                #                 batch_X=copy.deepcopy(actions_list), 
                #                 n_encode_lstm_step=n_encode_lstm_step, 
                #                 dim_wordvec=dim_wordvec,
                #                 word_vector=word_vector)
                tokens_input_action, tokens_length_action = make_batch_X_elmo(
                                batch_X=copy.deepcopy(actions_list), 
                                n_encode_lstm_step=n_encode_lstm_step)

                # convert the actions to Y
                ...
                # ease of answering
                # you measure the action for ease of answering, the data from dull_set is an example of response to a confusing sentence? 
                dull_loss = []
                for _id in range(len(tokens_input_action)):
                    # action_batch_X = np.array([vector for _ in range(batch_size)])
                    action_batch_X_token_input = [tokens_input_action[_id]] * batch_size
                    action_batch_X_token_length = [tokens_length_action[_id]] * batch_size

                    d_loss = sess.run(loss,
                                 feed_dict={
                                    # input_tensors['word_vectors']: action_batch_X,
                                    input_tensors['tokens_input']: action_batch_X_token_input,
                                    input_tensors['tokens_length']: action_batch_X_token_length,
                                    input_tensors['caption']: dull_matrix,
                                    input_tensors['caption_mask']: dull_mask,
                                    input_tensors['reward']: ones_reward
                                })

                    # the higer the d_loss, the better? (since your sentence doesn't match the preset responses)
                ...
                # semantic coherence
                # measure the loss using if we are taking the actions from earlier
                forward_inter = sess.run(inter_value,
                                 feed_dict={
                                    # input_tensors['word_vectors']: current_feats,
                                    input_tensors['tokens_input']: tokens_input,
                                    input_tensors['tokens_length']: tokens_length,
                ...
                # former is the sentence before the question (my guess)
                ...
                #                 dim_wordvec=dim_wordvec,
                #                 word_vector=word_vector)
                action_feats_tokens, action_feats_lengths = make_batch_X_elmo(
                                batch_X=copy.deepcopy(actions_list), 
                                n_encode_lstm_step=r_n_encode_lstm_step)

                # inputs are the action, measure the loss to the question that lead to the action
                backward_inter = sess2.run(reverse_inter,
                                 feed_dict={
                                    # word_vectors: action_feats,
                                    tf_reversed_tokens_input: action_feats_tokens,
                                    tf_reversed_tokens_length: action_feats_lengths,
                ...
                if batch % 1000 == 0 and batch != 0:
                    ...
                    saver.save(sess, os.path.join(model_path, 'model-RL-{}-{}'.format(epoch, batch)))
        ...
        saver.save(sess, os.path.join(model_path, 'model-RL'), global_step=epoch)
